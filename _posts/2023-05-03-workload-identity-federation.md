...
To illustrate the feature, we will setup a complete example with a simple Spring Boot application that will connect an Azure database for PostgreSQL in a passwordless model.
...
- issuer: this is the URL which correspond to the IdP. It is used by Azure AD to get keys and control that tokens it receives have been provided by this IdP.
...
A gist which go through all steps is available [here](https://gist.github.com/alexisP/88222ce7a6ed6188e726186f4b763e23)
...
For the Spring Boot application, i directly used the one issued from the sample from Azure-Samples in github (https://github.com/Azure-Samples/quickstart-spring-data-jpa-postgresql) which illustrate how to make a Spring Boot application connected to an Azure database for PostgreSQL and I adapted its configuration to make it compatible with a passordless authentication through a managed identity. To adapt this application, you need to modify two elements:
...
I will not go through the details on how the applications has been built, packaged and deployd in this article. Feel free to go to this [Github repository](https://github.com/alexisP/spring-boot-demo/tree/passwordless) for more information (last part of the README file). We will assume for the rest of the article that this application is available as a Docker image at the following URL: `aplantin.azurecr.io/spring-boot/spring-boot-demo-workload-identity-passwordless:1.2`
...
Yes, we dit it! We have our Spring Boot application deployed in a Kubernetes cluster which connects to an Azure database for PostgreSQL without any credentials stored anywhere.
...
Let's consider that you have an EKS cluster up & running in AWS. By default, you already have an OpenID provider setup and associated with the cluster. As you can see on the screenshot below, you can get the URL from the Overview tab. As a reminder, this URL is used to setup our federated credential.
